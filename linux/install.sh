...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo -e "===================================================${NC}"
    echo ""
    echo "To start your AI, run:  bash linux/start-linux.sh"
    echo "Or make it executable:  chmod +x linux/start-linux.sh && ./linux/start-linux.sh"
    echo "Tip: Run bash linux/preflight-check.sh first on a new drive to validate it."
else
